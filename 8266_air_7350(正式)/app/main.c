...
void Detect(void)
{
	...
	if(++ProgramPara.Cnt_Warning3 <= 100)//2s
	{
		if(ProgramPara.Voltage <= 15)//小于1.5V
		{	
	...
	else
	{
		...
		if(ProgramPara.Cnt_Warning0 >= 75)
		{
			LED_Ctrl(200,BLUE);
			LED_Ctrl(200,RED);
			ProgramPara.flag_Alarm = 0;
		}
		else if(ProgramPara.Cnt_Warning1 >= 75)
		{
		...
		else if(ProgramPara.Cnt_Warning2 >= 75)
		{
				LED_Ctrl(200,GREEN);
				LED_Ctrl(200,BLUE);
				//breath_switch = 1;
